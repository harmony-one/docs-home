...
**2.** Before continuing, check if your node is fully synced using an external endpoint. Replace parameter `--node` with the enpoint you want to compare with. Check the examples below  for shard 0 on mainnet and testnet. Just change the edpoints to compare with other shards.
...
**3.** Verify the blocks shown on steps 1 and 2 are closed or equal to each other.
